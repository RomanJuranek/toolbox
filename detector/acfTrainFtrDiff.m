...
for stage = 0:numel(opts.nWeak)-1
  ...
  if( stage==0 )
    ...
    % Instead of using X1 (channels) directly as features, we compute pixel
    % differences and use it as features. What is computed, can be
    % controlled by pFtrs structure (however pixel difference only is supported in mex code).
    F = ftrsGenerate( size(X1), opts.pFtrs );
    X1 = ftrsCompute( X1, F, opts.pFtrs ); % chns -> features
  ...
  X0 = ftrsCompute( X0, F, opts.pFtrs );
  ...
  % fids now correspond to to a pair of pixels.
  % fids -> fids1, fids2 which is used by mex code to access pixels
  fids=detector.clf.fids; [N,T]=size(fids);
  detector.clf.fids1=zeros(N,T,'int32');
  detector.clf.fids2=zeros(N,T,'int32');
  for t=1:T,
      j=detector.clf.child(:,t)~=0;
      detector.clf.fids1(j,t)=F(1,fids(j,t)+1)-1;
      detector.clf.fids2(j,t)=F(2,fids(j,t)+1)-1;
  end
...
dfs= { 'pPyramid',{}, 'filters',[], ...
  ...
  'nPerNeg',25, 'nAccNeg',10000, 'pJitter',{}, 'pFtrs',{}, 'winsSave',0 };
...
dfs={'nFtrs',10000,'ftrfun',@minus};
opts.pFtrs=getPrmDflt(opts.pFtrs,dfs,1);
...
fprintf('Computing channels... '); start=clock; fs=opts.filters;
...
function ftrs = ftrsGenerate( dims, pFtrs )
% Generate n random pixel comparison features.
% TODO: Constraints
% - Channels that can be compared by one ftr (useless to compare Y and gradient)
% - Spatial distance between pixels
n=prod(dims(1:2)); ftrs=randi(n,2,pFtrs.nFtrs);
nchns=dims(3); c=randi(nchns,1,pFtrs.nFtrs)-1;
ftrs=bsxfun(@plus,ftrs,n*c);
end

function f = ftrsCompute( X, ftrs, pFtrs )
% Eval features F on samples X using function fun (subtraction by default).
if isempty(X), f=[]; return; end;
fprintf('Extracting features... '); start=clock;
if ~isfield(pFtrs,'ftrfun') || isempty(pFtrs.ftrfun), ftrfun=@minus;
else ftrfun=pFtrs.ftrfun; end;
%{
% This works, but not much readable
sz=size(X); stride=prod(sz(1:3));
seq=repmat((0:stride:(numel(X)-1))',1,size(ftrs,2));
is=bsxfun(@plus,seq,ftrs(1,:)); js=bsxfun(@plus,seq,ftrs(2,:));
f=single(feval(ftrfun,X(is),X(js)));
%}
n=size(X,4); nftrs=size(ftrs,2); f=zeros(n,nftrs,'single');
for i = 1:n
    x = X(:,:,:,i);
    f(i,:) = feval(ftrfun,x(ftrs(1,:)),x(ftrs(2,:)));
end
fprintf('done (time=%.0fs).\n',etime(clock,start));
end
